...
## App identity
- **Package name (App ID):** `co.riksu.shc`
...
3. Set **Package ID** = `co.riksu.shc` (must match `assetlinks.json`).
...
--manifest https://shc.riksu.co/manifest.json`, set the App ID to `co.riksu.shc`, then
