...
def get_stats(db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    today = today_ist()
    visits_month    = db.query(FieldReport).filter(FieldReport.report_date >= date(today.year, today.month, 1)).count()
    low_stock       = db.query(StockItem).filter(StockItem.min_qty > 0, StockItem.office_qty <= StockItem.min_qty, StockItem.is_active == True).count()
...
def recent_visits(db: Session = Depends(get_db), _=Depends(get_current_user)):
    reports = db.query(ServiceReport).order_by(ServiceReport.report_date.desc()).limit(10).all()
    return [{
        "id": r.id, "school": r.school.name if r.school else "—",
        "employee": r.employee.name if r.employee else "—",
        "date": r.report_date.isoformat() if r.report_date else None,
        "tds": r.tds_input, "ph": None, "type": r.unit_type
    } for r in reports]

@router.get("/alerts")
def get_alerts(db: Session = Depends(get_db), _=Depends(get_current_user)):
    alerts = []
    low_stock = db.query(StockItem).filter(StockItem.min_qty > 0, StockItem.office_qty <= StockItem.min_qty, StockItem.is_active == True).all()
